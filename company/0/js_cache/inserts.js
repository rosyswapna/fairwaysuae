...
function _set_combo_input4(e){
	...
	e.onkeyup=function(ev){
		...
		var autoText=document.getElementsByName('autofill_search')[0];

		if(string_contains(this.className,'autofill_search')){
			show_autofill();
			var searchTxt = autoText.value;
			
			JsHttpRequest.request(this);
			if(JsHttpRequest.responseJS){
				alert("return data");
			}
			...
			if(select&&select.selectedIndex>=0){
				var len=select.length;
				var byid=string_contains(this.className,'autofill_search');
...
function _update_box(s){
	var byid=string_contains(s.className,'combo')||string_contains(s.className,'combo3')||string_contains(s.className,'combo4');
...
function _set_combo_select4(e) {
		...
		e.onblur = function() {
		    	var box = document.getElementsByName(this.getAttribute('rel'))[0];
			hide_autotext_input(box);
				
			if ((this.selectedIndex != this.getAttribute('_last'))
				||(string_contains(this.className, 'combo4') && _update_box(this))
				)
					this.onchange();
		}
		e.onchange = function() {
			var box_search = document.getElementsByName('autofill_search')[0];
			var box = document.getElementsByName(this.getAttribute('rel'))[0];
			var s = this;
			this.setAttribute('_last', this.selectedIndex);
			if(string_contains(s.className, 'combo4')){
...
{
	document.getElementsByClassName('autofill')[0].style.display = 'none';
}
function show_autofill()
{

	document.getElementsByClassName('autofill')[0].style.display = 'block';
		
}

function show_autotext_input(obj)//supply actual element
{
	//alert(obj.value);
	obj.style.display = 'none';
	var autoText=document.getElementsByName('autofill_search')[0];
	autoText.value = obj.value;
	autoText.style.display = 'block';
}

function hide_autotext_input(obj)//supply actual element
{
	var autoText=document.getElementsByName('autofill_search')[0];
	...
	function(e){
		if(e.onfocus==undefined){
			e.onfocus=function(){
				...
				else if(string_contains(this.className,'combo4'))
				{

					show_autotext_input(this);
					e.onblur();return false;
...
}else if(string_contains(e.className,'autofill_search')){
	_set_combo_input4(e);
}
//-----------------------------------
...
//------------------------------
if(string_contains(c,'combo4'))
	_set_combo_select4(e);
//--------------------------------

else{e.onkeydown=function(ev){ev=ev||window.event;key=ev.keyCode||ev.which;if(key==8||(key=37&&ev.altKey)){ev.returnValue=false;return false;}
}
}
},
...
//------------------------------------------
'.dropdown_li': function(l) {
		l.onclick = function() {

			var autoText=document.getElementsByName('autofill_search')[0];

			var itemCode = this.getAttribute('_get_val');
		    	var autofill_ul = this.parentElement.parentElement;
			
			var setForId = autofill_ul.getAttribute('set_id');
			var edit_box = document.getElementById(setForId);
			var list_box = edit_box.getAttribute('rel');
			edit_box.value = itemCode;
			autofill_ul.style.display = "none";
			autoText.style.display = "none";
			document.getElementsByClassName('combo4')[0].style.display = 'block';
			document.getElementsByName(list_box)[0].value = itemCode;
