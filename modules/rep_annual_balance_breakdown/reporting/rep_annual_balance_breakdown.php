...
// Date:	2009-12-22
// Title:	Annual Balance Breakdown - Detail
// ----------------------------------------------------------------
$path_to_root="..";
...
print_annual_balance_breakdown_detail();
...
function display_type ($type, $typename, $yr, $mo, $convert, &$dec, &$rep, $dimension, $dimension2)
{
	...
	if ($printtitle)
	{
		...
		$rep->SetCellPadding(0);
		$fill = 0;
		$rep->TextCol(0, 1,	_('Total') . " " . $typename, 0, 4, 0, $fill, NULL, 1);
		$rep->SetCellPadding($oldcMargin);
		for ($i = 1; $i <= 12; $i++)
...
function print_annual_balance_breakdown_detail()
{
	...
	$title = _("Assets Vs Liabilities Comparison - Detailed");
   	//$output_filename = "AnnualBalanceBreakDownDetail";
	$output_filename = "AssetsVsLiabilitiesComparison";
   	$fontsize = 7;
	...
	$enddate = end_month($date);
   	...
   	$wi = ($endline - $cols2[1]) / 12; // 12 amount columns
   	for ($i = 2; $i < 15; $i++)
